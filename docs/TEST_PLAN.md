...
|---|---|---|
| 3.0 home base | `base.test.luau` | Build/upgrade through the purchase path, Hearth gates, max level, insufficient coins, read-only and in-run refusals; tiles and swaps; tampered layouts and buildings bought on another server; offline claim math, cap and two-server idempotency; auto-claim before a yard/Storehouse upgrade; cheers once a day with caps and rollover; bonus clamping; the Outpost's plots and prompts; the base remotes' sanitising. |
| 3.0 run changes | `runloop.test.luau` | Bank loot door (last, only after extract points, depth bonus, tie keeps descending); half the haul on defeat; 7 daily modifiers (stats, enemies, champions, once-a-day bonus); the first descent (plan, clustered opener, forced Split Shot, fusion "try it now", flag set); card labels/icons, RECOMMENDED, unlock levels, respec, reroll (free then tokens); party and friend bonus, enemy scaling, FriendCache; facing on arrival. |
| Depth pressure | `pressure.test.luau` | Swift hunters after 60s in a normal room (countdown, no coins, no champion credit, never hold the room, MaxAlive cap, leave on clear); objective rooms and mini-bosses exempt; a timed-out room loses its door reward; off unless the route asks; HUD countdown. |
| 3.0 economy | `economy.test.luau`, `monetization.test.luau`, `shop.test.luau` | Presets (slots, round trip, owned trails only); revive tokens (grace window, once a run, spend through the purchase path); rewarded video (flag, daily cap, skipped/failed ads pay nothing); private-server owner; passes as entitlements, products as tokens, the no-paid-power rule, catalogue prices; cosmetics by coins/level/pass, equip by kind, emotes, nameplate titles. |
| 3.0 consumables and migration | `currencies.test.luau` | Rerolls and ReviveTokens through grants and spends, cross-server spend rejection, the v8 → v9 Armory refund exactly once. |
| 3.0 telemetry | `funnel.test.luau` | Funnel steps once ever (across servers), read-only reports nothing, AnalyticsService failures swallowed, economy sources and sinks from PlayerDataService, run-loop steps. |
| 3.0 views | `views.test.luau` | Base panel (grid, tap-to-build/move, claim, visits), card picker (icons, labels, RECOMMENDED, reroll, respec, fusion, Bank loot door), results (haul, next build, CTAs), shop tabs by kind, HUD BASE/modifier/revive, menu BASE, presets, base preview camera, daily extras, aim hint pulse, onboarding hints. |
...
| Profile schema | `persistence.test.luau` | Defaults from nothing, missing fields filled without loss, idempotent normalisation, corrupt stored values survived, and migration from every older version to v4. |
| Harness scheduler | `scheduler.test.luau` | Spawned threads run to their first yield; `task.wait` suspends and wakes in time then scheduling order; main-thread waits let time pass; delay, defer, cancel and error capture; the fake DataStore's latency, value freezing and conflict re-runs. |
| Persistence lifecycle | `persistence_lifecycle.test.luau` | One section per persistence plan step: payouts written on grant; no change lost during an in-flight save; concurrent saves coalescing; stale loads discarded and rejoin takeover; cross-server grants composing, time-ordered and capped ledger; parallel shutdown under a deadline with isolated workers. Every regression test failed against the code before its step. |
